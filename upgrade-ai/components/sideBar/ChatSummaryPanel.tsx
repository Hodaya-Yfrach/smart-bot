...
// =============================================================================
// components/sideBar/ChatSummaryPanel.tsx — פאנל תקציר שיחה
//
// זרימת עבודה:
//   1. פתיחה → טוען תקציר מ-DB (chat_summaries).
//   2. אם אין תקציר ב-DB → שולח בקשה ל-AI ושומר ב-DB.
//   3. כפתור 🔄 → מייצר מחדש ומעדכן ב-DB (upsert).
//   4. מחיקת שיחה → CASCADE ב-DB מוחק את התקציר אוטומטית.
//
// DEV NOTE: הפאנל עצמו מנהל את הטעינה/שמירה מ-DB — page.tsx אינו מעורב.
// =============================================================================
...
export default function ChatSummaryPanel({
  ...
  // עוקב אחרי ה-chatId שעבורו כבר טענו תקציר — מונע בקשות כפולות
  const loadedForChatId = useRef<string | null>(null);

  // ── טעינת תקציר מ-DB ────────────────────────────────────────────────────────
  const loadFromDb = async (): Promise<ChatSummary | null> => {
    ...
    const { data } = await supabase
      ...
      .eq('chat_id', chatId)
      .maybeSingle();
    return (data?.summary as ChatSummary) ?? null;
  };
  ...
  useEffect(() => {
    ...
    // אם כבר טענו לשיחה הזו באותה session — אל תטען שוב
    ...
    const init = async () => {
      ...
      try {
        ...
        if (existing) {
          // יש תקציר ב-DB — מציגים אותו ישר, ללא קריאה ל-AI
          setSummary(existing);
          loadedForChatId.current = chatId;
          setIsLoading(false);
        } else {
          // אין תקציר — יוצרים ושומרים
          setIsLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" dir="rtl">
      ...
      <div className="relative bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.08)] w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden animate-fade-in border border-slate-100">
        ...
        <header className="px-6 pt-7 pb-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={isLoading}
              title="יצירת תקציר מחדש (מעדכן את ה-DB)"
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
          ...
          {!isLoading && error && (
            <div className="text-center text-sm font-medium text-red-600 bg-red-50 border border-red-100 p-4 rounded-2xl">
              {error}
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="border border-teal-100/60 bg-white rounded-2xl overflow-hidden shadow-sm">
                ...
                >
                  ...
                  <span className="text-[11px] font-bold text-teal-600/80 bg-teal-50 px-2.5 py-1 rounded-md">
                    {showOverall ? '▲ הסתר' : '▼ הצג'}
                  </span>
                </button>
                {showOverall && (
                    ...
                    {!summary.oneLineSummary && summary.overallSummary.length === 0 ? (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        {summary.overallSummary.length > 0 && (
                          <button onClick={() => setShowExpandedOverall(true)}
                            className="text-xs font-bold text-teal-600 hover:underline shrink-0 mt-1">
                            הרחב פירוט
                          </button>
                        )}
                      </div>
                      ...
                      <div className="space-y-2 text-slate-600 font-medium animate-fade-in">
                        ...
                        <button onClick={() => setShowExpandedOverall(false)}
                          className="text-xs font-bold text-teal-600 hover:underline mt-2 block">
                          חזרה לקיצור
                        </button>
                      </div>
                    )}
